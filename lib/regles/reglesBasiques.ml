...
let init_pos fen = EntreeSortie.Fen.creer_partie_fen fen, ()
...
let attaquee_dir partie couleur pos =
  List.concat_map
    (fun p ->
      ...
      |> List.filter
           (List.exists (contient partie.echiquier (inverse couleur, p))))
...
let deplacer_piece partie ((x, y) as dep) ((x', y') as arr) =
  ...
  { partie with echiquier; en_passant }
...
let roque partie type_roque =
  if peut_roquer partie type_roque then
    ...
    let partie = deplacer_piece partie (x_tour, y) (x + type_roque, y)
    in
...
let case_depart_pion partie (x, y) =
  if
    Option.fold partie.en_passant ~none:false ~some:(( = ) (x, y))
    || est_adversaire partie.trait partie.echiquier.${x, y}
  then case_depart_autre partie Pion (x, y)
  else
    match partie.trait with
    | Blanc ->
        if y > 0 && contient partie.echiquier (partie.trait, Pion) (x, y - 1)
        then [ x, y - 1 ]
        else if
          y = 3 && contient partie.echiquier (partie.trait, Pion) (x, y - 2)
        then [ x, y - 2 ]
        else []
    | Noir ->
        if y < 7 && contient partie.echiquier (partie.trait, Pion) (x, y + 1)
        then [ x, y + 1 ]
        else if
          y = 4 && contient partie.echiquier (partie.trait, Pion) (x, y + 2)
        then [ x, y + 2 ]
        else []
