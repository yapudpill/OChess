...
| Echiquier.Petit_roque -> "O-O"
| Echiquier.Grand_roque -> "O-O-O"
| Echiquier.Mouvement (p, (x, y)) ->
...
| "o-o" | "O-O" -> Some Echiquier.Petit_roque
| "o-o-o" | "O-O-O" -> Some Echiquier.Grand_roque
| _ ->
  let len = String.length str in
  if len <> 2 && len <> 3 then None
  else
    let p =
      if len = 2 then Some Piece.Pion
